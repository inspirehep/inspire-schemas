...
def test_latex_to_unicode_preserves_math():
    expected = '$H_{\\text{Schr\\"{o}dinger}}$'
    result = ArxivParser.latex_to_unicode('$H_{\\text{Schr\\"{o}dinger}}$')

    assert result == expected


def test_latex_to_unicode_preserves_braces_containing_more_than_one_char():
    expected = "On the origin of the Type~{\\sc ii} spicules - dynamic 3D MHD simulations"
    result = ArxivParser.latex_to_unicode(
