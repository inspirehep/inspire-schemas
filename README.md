...
Start a shell inside the container, then run the tests:
...
The image build already installs dependencies and generates the schema JSON
files, so the container is ready to test as soon as it starts.
...
The `generate_schemas` script resolves the `.yml` schema sources under
`inspire_schemas/records` into the `.json` files the library loads at
runtime; it must be re-run after pulling changes to any schema.
