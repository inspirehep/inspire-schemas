...
_RE_LICENSE_URL = re.compile(r"^/licenses/(?P<sublicense>[-\w]*)(?:/(?P<version>[\.\d]*))?")
_RE_VOLUME_STARTS_WITH_A_LETTER = re.compile(
    r"^(?P<letter>[A-Z])(?P<volume>\d[\dA-Z-]*$)", re.IGNORECASE
)
_RE_VOLUME_ENDS_WITH_A_LETTER = re.compile(r"(?P<volume>\d+)(?P<letter>[A-Z])$", re.IGNORECASE)
_RE_TITLE_ENDS_WITH_A_LETTER = re.compile(r"(?P<title>.+(\.| ))(?P<letter>[A-Z])$", re.IGNORECASE)
...
RE_ARXIV_POST_2007_CLASS = re.compile(r"(arxiv:)?{}".format(RE_ARXIV_POST_2007), flags=re.I)

RE_ARXIV_PRE_2007 = r"(?P<category>(?P<extraidentifier>[a-z-]+)(?:\.[a-z]{2})?)/(?P<identifier>\d{4}\d+)(v\d+)?\s*(\[(?:[a-z\-\.]+)\])?$"  # noqa: E501
RE_ARXIV_PRE_2007_CLASS = re.compile(r"(arxiv:)?{}".format(RE_ARXIV_PRE_2007), flags=re.I)
...
def filter_empty_parameters(func):
    ...
    def func_wrapper(self, *args, **kwargs):
        my_kwargs = {key: value for key, value in kwargs.items() if value not in EMPTIES}
        args_is_empty = all(arg in EMPTIES for arg in args)

        if ({"source", "material"}.issuperset(my_kwargs) or not my_kwargs) and args_is_empty:
            return
        ...
        if (
            ...
            try:
                ...
                _publication_info["year"] = int(
                    "19" + journal_volume[:2] if was_last_century else "20" + journal_volume[:2]
                )
...
def normalize_collaboration_name(full_collaboration_string):
    ...
    compiled_regexp = re.compile(r"\b(" + "|".join(words_to_ignore) + r")\b", flags=re.IGNORECASE)
